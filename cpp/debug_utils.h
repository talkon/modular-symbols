...
#define DEBUG_INFO_PRINT(v, ...) if (verbosity >= v) { info_with_time(); printf(__VA_ARGS__); check_status(); }

#define DEBUG_INFO(v, ...) if (verbosity >= v) { info_with_time(); __VA_ARGS__ check_status(); }
